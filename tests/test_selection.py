...
class SelectionTests(unittest.TestCase):
    ...
    def test_quick_export_filter_matches_table_semantics(self):
        payload = json.loads(self.db.execute("SELECT payload FROM results WHERE proxy='https://11.0.0.4:443'").fetchone()[0])
        payload['speed'] = {'mbps': 12.5}
        self.db.execute("UPDATE results SET payload=? WHERE proxy='https://11.0.0.4:443'", (json.dumps(payload),))
        self.db.commit()
        report, proxies = self.exported(quick='speed')
        self.assertEqual((report['quick'], report['exported']), ('speed', 1))
        self.assertEqual(proxies, ['https://11.0.0.4:443'])
        _, http_rows = self.exported(quick='http')
        self.assertEqual(http_rows, ['https://11.0.0.4:443', 'http://11.0.0.1:80'])
