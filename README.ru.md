...
**Собирает бесплатные публичные прокси из 47 открытых списков, проверяет каждый на _ваших_ сервисах и оставляет только быстрые, стабильные и чистые.**

Локальный GUI в браузере + CLI · HTTP / HTTPS (CONNECT) / SOCKS5 · продолжение после остановки · без аккаунтов и телеметрии
...
[Быстрый старт](#-быстрый-старт) · [Возможности](#-возможности) · [Командная строка](#запуск-через-командную-строку) · [Чистота](#проверка-чистоты-и-локальный-denylist) · [FAQ](#-частые-вопросы) · [Планы](#-планы)
...
- Ранжирует прошедшие по **медианной задержке, разбросу и доле успехов**, отмечает **адреса из блеклистов** (локальный denylist + опциональные DNSBL) и экспортирует TXT / CSV / JSON.
...
<table>
<tr>
<td width="50%"><img src="docs/assets/screenshots/scan-dark.png" alt="Настройка и прогресс проверки"><br><sub><b>Проверка</b> — сервисы, правила успеха и живой прогресс</sub></td>
<td width="50%"><img src="docs/assets/screenshots/results-dark.png" alt="Рейтинг прокси"><br><sub><b>Результаты</b> — рейтинг по качеству или скорости с оценкой чистоты</sub></td>
</tr>
<tr>
<td width="50%"><img src="docs/assets/screenshots/details-dark.png" alt="Детали попыток"><br><sub><b>Детали</b> — каждая попытка по каждому сервису</sub></td>
<td width="50%"><img src="docs/assets/screenshots/results-light.png" alt="Светлая тема"><br><sub><b>Светлая тема</b> — переключается одной кнопкой</sub></td>
</tr>
</table>
...
| | |
| --- | --- |
| **47 встроенных источников** | Популярные списки на GitHub, ProxyScrape, постраничный API Geonode. Всё редактируется в `sources.json` или в GUI; можно добавить свои URL и TXT-файлы. |
| **Протоколы** | HTTP, HTTPS/CONNECT, явные `https://`-прокси, SOCKS5 / SOCKS5h, IPv4 и IPv6. |
...
| **Большие списки** | Ограниченная очередь, лимит частоты запросов, автоподбор числа воркеров. Проверено на 190 000 синтетических кандидатов. |
| **Остановка и продолжение** | Прогресс хранится в SQLite. `Ctrl+C` или **Остановить** сохраняет готовое; повторный запуск продолжает. |
| **Рейтинг и экспорт** | Сортировка `quality` или `speed`, топ-N или все в `proxies.txt`, `ranked.csv`, `ranked.json`. Crash-safe поколения экспорта. |
...
На Windows нужен Python 3.11+ с Python Launcher (`py`). Первый запуск создаст `.venv/` и установит зависимость, затем откроется локальная страница в вашем браузере. По умолчанию включена тёмная тема; кнопка вверху переключает на светлую. Выбор сохраняется в браузере. Сервер доступен только на этом устройстве. Повторный запуск открывает уже работающий интерфейс.
...
- [ ] Английский интерфейс и переключатель языка
- [ ] Установка через `pipx` / PyPI и единая команда `proxy-workbench`
- [ ] Определение уровня анонимности (transparent / anonymous / elite) через собственный echo-endpoint
- [ ] Колонка страны и фильтр по GeoIP из локальной базы
- [ ] Плановая перепроверка лучших прокси
- [ ] Docker-образ для серверов без GUI
