...
**Собирает бесплатные публичные прокси из каталога на 150 записей, включая 98 доступных для сбора лент, проверяет каждый на _ваших_ сервисах и оставляет только быстрые, стабильные, чистые и анонимные.**
...
| **Ротирующий прокси-шлюз** | Укажите `127.0.0.1:8899` как HTTP- или SOCKS5-прокси в браузере, Telegram, парсере или любой программе. Каждое новое соединение уходит через следующий рабочий прокси; неработающие автоматически пропускаются. |
| **Сначала рекомендуемые** | По умолчанию первыми идут прокси, которые быстрые, переживают перепроверки, пришли из списков с хорошей историей и встречаются в немногих списках: ими меньше пользуются, поэтому живут дольше. |
| **Скрипты и другие клиенты** | `proxy-workbench get --country DE --top 5` выводит рабочие прокси, `proxy-workbench test socks5://…` проверяет ваши; конфиг sing-box и кнопка **Открыть в Telegram** для ротирующего прокси. |
| **Для парсеров** | В имени пользователя шлюза выбирается всё нужное: `country-de`, `protocol-socks5`, `latency-800`, `session-abc` (один IP на сессию). Лимит соединений на прокси и статистика `/status`. |
| **Реальная скорость и провайдер** | Необязательный замер скорости в Мбит/с и сортировка по ней; у каждого прокси виден провайдер (ASN), хостинг и дата-центры можно пропускать. |
| **Готовые проверки** | Один клик добавляет проверку Google, YouTube, Telegram, Discord, Instagram, OpenAI API, GitHub, Wikipedia или Cloudflare. |
...
```sh
proxy-workbench                  # приложение: интерфейс + меню-бар + один экземпляр
proxy-workbench gui              # только интерфейс, без меню-бара
proxy-workbench --no-desktop     # то же самое, другими словами
...
По умолчанию: HTTPS-запрос к `https://example.com/`, 3 независимых замера на адрес, таймаут 8 секунд на запрос, до 100 стартов запросов/секунду. Нужны успешные ответы в двух из трёх попыток. При ограничении файловых дескрипторов число воркеров автоматически снижается.

### Потолок параллелизма и бюджеты прогона

`--workers 128` — это **потолок**, а не число одновременных проверок. Конвейер сам решает, сколько проверок ему нужно: при коротком списке кандидатов, при низком лимите скорости или при нехватке файловых дескрипторов одновременных проверок будет меньше, и значение `--workers` просто не будет достигнуто. Поднимать его выше имеет смысл только тогда, когда прогон действительно упирается в параллелизм.

```sh
./run.sh run --workers 256 --rate 100          # потолок и скорость
./run.sh run --max-requests 500000             # потолок запросов на один прогон
./run.sh run --run-max-bytes 2147483648        # потолок байт на один прогон
./run.sh run --want 20 --count-what exit       # что именно считает --want
```

- `--max-requests N` — бюджет запросов на прогон, `0` — без потолка. Когда бюджет исчерпан, прогон **останавливается**; непроверенные адреса не записываются как недостижимые.
- `--run-max-bytes N` — бюджет байт на прогон, `0` — без потолка; правило остановки то же.
- `--want N` — остановиться, когда найдено N годных прокси; `0` (по умолчанию) — проверять всё.
- `--count-what endpoint|ip|exit` — что именно считает `--want`: сами адреса (`endpoint`, по умолчанию), уникальные IP за ними или IP, подтверждённые как адрес выхода. Это три разных числа, и `endpoint` достичь проще всего.

### `bench`: замер самого конвейера

```sh
./run.sh bench --bench-items 2000 --bench-n 20
```

Команда прогоняет конвейер на **локальной синтетической фикстуре**: без сокетов, без DNS и без публичных прокси. Она печатает пропускную способность, время до первого результата и время до N. Числа от `bench` — это измерение конвейера, а не измерение интернета, и заметка об этом печатается вместе с каждым числом.
...
`--workers` здесь — потолок, а не число одновременных проверок: конвейер сам решает, сколько ему нужно, и до 256 одновременно дойдёт только тот прогон, которому это действительно требуется. Бюджеты прогона и команда `bench` описаны выше.
...
## Ротирующий прокси-шлюз

Пока открыт GUI, адрес `127.0.0.1:8899` работает как локальный прокси, который по очереди использует все рабочие прокси из последнего экспорта. По умолчанию GUI также поднимает защищённый LAN-шлюз, чтобы телефон в той же сети мог использовать QR для Telegram: QR содержит адрес компьютера в сети и пароль текущей сессии. Разрешите порт в локальном брандмауэре. Если LAN не нужен, запустите `gui --gateway-host 127.0.0.1`.

Локальный шлюз поддерживает только **HTTP/SOCKS5 TCP**. SOCKS5 UDP ASSOCIATE, звонки Telegram и игры, которым нужен UDP, здесь не реализованы; для них нужен отдельный VPN/туннель.
...
- Каждое новое TCP-соединение берёт следующий прокси (`--rotate random` выбирает случайно).
...
| `GET /pac` | автонастройка прокси для браузера с 10 лучшими подходящими прокси |
| `GET /clash` | конфиг Clash / Mihomo с группой автоматического выбора |
...
В JSON у каждого прокси есть `proxy`, `protocol`, `host`, `port`, `country`, `exit_ip`, `exit_country`, `anonymity`, `latency_ms`, `jitter_ms`, `reliability`, `uptime`, `checks`, `score`, `checked_at`. В GUI адрес API с кнопкой **Скопировать** находится под кнопками скачивания; порт меняется флагом `gui.py --api-port 9000`, выключить API можно флагом `--no-api`.
...
- `proxy_workbench/` — сам пакет:
  ...
  - `desktop.py` — фоновый слой поставки (F22): per-user `data`/`cache`/`logs`, portable mode, перенос старой папки, один экземпляр, меню-бар на macOS, запуск при входе, сон/пробуждение;
