...
| **Рейтинг и экспорт** | Сортировка `quality` или `speed`, топ-N или все в `proxies.txt`, `ranked.csv`, `ranked.json`, а также `http.txt` / `https.txt` / `socks5.txt` в формате `host:port` для других программ. Crash-safe поколения экспорта. |
