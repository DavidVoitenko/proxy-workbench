...
**Собирает бесплатные публичные прокси из 47 открытых списков, проверяет каждый на _ваших_ сервисах и оставляет только быстрые, стабильные, чистые и анонимные.**
...
| **Рейтинг и экспорт** | Сортировка `quality`, `speed` или `stability`; фильтры по протоколу, стране, максимальной задержке, анонимности и успешности; поиск по адресу или порту и копирование страницы в один клик. Экспорт топ-N или всех в `proxies.txt`, `ranked.csv`, `ranked.json`, а также `http.txt` / `https.txt` / `socks5.txt` в формате `host:port` для других программ. Crash-safe поколения экспорта. |
...
- Порядок проверки: сначала адреса, которые уже проходили в других профилях, остальные — в случайном порядке, чтобы первые находки не были из одной подсети или одного источника.
