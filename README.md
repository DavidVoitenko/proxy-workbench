...
Первый запуск создаст `.venv` и установит зависимость. Затем загрузит публичные списки из `sources.json`, удалит дубликаты и проверит **все** адреса. Число кандидатов зависит от источников: 190 тысяч не гарантированы. Нет ограничения на количество проверяемых адресов, общего таймера прохода, остановки при заполнении пула, фильтров по стране, ChatGPT или инвойсам.
...
URL, содержимое, заголовки, число попыток, таймаут и размер ответа определяют профиль. Изменение этих параметров создаёт отдельный набор результатов. Повторный запуск того же профиля продолжает проход; для обновления старых оценок нужен `--recheck`. Прерванный посреди попыток адрес проверяется заново. Завершённые результаты пишутся в SQLite пакетами; Ctrl+C сохраняет текущий пакет, аварийное завершение может потребовать повторить последний пакет.
...
Тесты используют временные базы, имитацию 190 000 кандидатов и локальный HTTP-прокси. Внешних сервисов и рабочих аккаунтов не касаются. Документация транспорта: [HTTPX proxy support](https://www.python-httpx.org/advanced/proxies/).
