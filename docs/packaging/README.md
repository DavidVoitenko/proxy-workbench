...
### Телефон в одной сети

GUI по умолчанию поднимает gateway на `0.0.0.0` с одноразовым паролем сессии.
В разделе Mobile QR должен быть показан реальный адрес компьютера в локальной
сети, а не `127.0.0.1`; телефон и компьютер должны быть в одной сети, а
брандмауэр — разрешать выбранный порт. Локальные клиенты на этом же компьютере
продолжают работать без пароля. Если LAN не нужен, запустите
`proxy-workbench gui --gateway-host 127.0.0.1`. Шлюз поддерживает HTTP и
SOCKS5 TCP; UDP ASSOCIATE, звонки и другие UDP-приложения требуют отдельного
туннеля.
