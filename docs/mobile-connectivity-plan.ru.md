...
Текущий sing-box-экспорт использует локальный mixed inbound и один общий URLTest; при пустом или просроченном списке он теперь завершает трафик через fail-closed `block`, а не создаёт DIRECT-выход. Это всё ещё не готовый семейный профиль с TUN/DNS/UDP: выбранные туннелируемые сервисы нужно проверить в конкретном клиенте и сети.
